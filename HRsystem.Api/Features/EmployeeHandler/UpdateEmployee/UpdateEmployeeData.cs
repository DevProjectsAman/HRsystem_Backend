...
namespace HRsystem.Api.Features.EmployeeHandler.UpdateEmployee
{
        ...
        : IRequestHandler<UpdateEmployeeOrganizationCommand, ResponseResultDTO>
    {
        ...
        public UpdateEmployeeOrganizationHandler(DBContextHRsystem db,ICurrentUserService currentUserService)
        {
            _db = db;
            ...
            CancellationToken ct)
        {
            ...
            // Remove old projects
            var existingProjects = await _db.TbEmployeeProjects
                .Where(ep => ep.EmployeeId == request.EmployeeId)
                .ToListAsync(ct);

            _db.TbEmployeeProjects.RemoveRange(existingProjects);

            // Add new projects
            if (request.ProjectId != null && request.ProjectId.HasValue)
            {
                var newProjects =  new TbEmployeeProject
                {
                ...
                await _db.TbEmployeeProjects.AddRangeAsync(newProjects);
            }
            await _db.SaveChangesAsync(ct);
        ...
        : IRequestHandler<UpdateEmployeeShiftWorkDaysCommand, ResponseResultDTO>
    {
        ...
        public UpdateEmployeeShiftWorkDaysHandler(DBContextHRsystem db ,ICurrentUserService currentUserService)
        {
            _db = db;
            _currentUserService = currentUserService;
        }
            ...
            CancellationToken ct)
        {
          ...
          //  employee.RemoteWorkDaysId = request.RemoteWorkDaysId;
            ...
            var existingShifts = await _db.TbEmployeeShifts
                .Where(es => es.EmployeeId == request.EmployeeId)
                .ToListAsync(ct);

            _db.TbEmployeeShifts.RemoveRange(existingShifts);

            // Add new shift assignment
            var newShift = new TbEmployeeShift
            {
                EmployeeId = request.EmployeeId,
            ...
            await _db.TbEmployeeShifts.AddAsync(newShift, ct);
