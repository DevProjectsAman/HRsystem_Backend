...
namespace HRsystem.Api.Features.Reports
{
    public class HomeDashboardReport
    {
        ...
        // ===================== VALIDATOR =====================
        public class GetDashboardReportQueryValidator
            : AbstractValidator<GetDashboardReportQuery>
        {
            public GetDashboardReportQueryValidator()
            {
        ...
        // ===================== HANDLER =====================
        public class GetDashboardReportHandler
            : IRequestHandler<GetDashboardReportQuery,
                ResponseResultDTO<HomeDashboardReportDto>>
        {
            private readonly DBContextHRsystem _db;
                ...
                CancellationToken cancellationToken)
            {
                try
                {
                    ...
                    // ===== Employees =====
                    var totalEmployees = await _db.TbEmployees.CountAsync(cancellationToken);
                    var activeEmployees = await _db.TbEmployees
                        .CountAsync(e => e.Status == "1", cancellationToken);
                    ...
                    // ===== Requests =====
                    var totalRequests = await _db.TbEmployeeActivities.CountAsync(cancellationToken);
                    var totalApprovedRequests = await _db.TbEmployeeActivities
                        .CountAsync(r => r.StatusId == 7, cancellationToken);
                    var totalPendingRequests = await _db.TbEmployeeActivities
                        .CountAsync(r => r.StatusId == 8, cancellationToken);
                    ...
                    // ===== Group by Department =====
                    var departmentStatus = employees
                        .GroupBy(e => new
                        {
                            e.DepartmentId,
                            DepartmentName = e.Department.DepartmentName.en
                        })
                        .Select(g =>
                        {
                            // هنا نستخدم EmployeeId كـ int
                            var employeeIds = g.Select(x => x.EmployeeId).ToList();

                            var statuses = attendanceStatuses.Select(s =>
                            {
                                var count = attendanceReports
                                    .Count(a => a.EmployeeId.HasValue && employeeIds.Contains(a.EmployeeId.Value) &&
                                                a.EmployeeTodayStatuesId == s.AttendanceStatuesId);

                                var percentage = (g.Count() * totalDays) == 0
                                    ? 0
                                    : Math.Round((double)count / (g.Count() * totalDays) * 100, 2);
                    ...
                    var dto = new HomeDashboardReportDto
                    {
                        ...
                        TotalApprovedRequests = totalApprovedRequests,
                        TotalPendingRequests = totalPendingRequests,
                        TodayDepartmentStatus = departmentStatus
                    };
